...
% In addition to these data sources, we also use ancillary data from the South Pole Imaging Fabry-Perot Interferometer (SPIFI) and the Infrared Satellite Observatory (ISO) to compare our results \citep{oberst2011205}.
...
On SOFIA was the German REceiver for Astronomy at Terahertz frequencies (GREAT) which observed a wide range of spectral lines between 1.25 and 2.5 THz using a heterodyne receiver \citep{heyminck2012great}.
...
Now we can look at the integrated intensity of the RRL line using the following equation \citep{pineda2018}:
\begin{equation}
...
Because the hydrogen recombination lines can be affected by non-local thermal equilibrium (NLTE) effects, we also need to account for the deviation from LTE in the RRL emission \citep{gordon2002radio}.
\begin{equation}
...
\begin{figure}
    ...
    \begin{subfigure}[t]{0.5\textwidth}
        ...
        \caption{Fractional Population of the $^3P_1$ Level of Ionized Nitrogen as a Function of Electron Density}
        \label{carina/fig:nii_curves}
    \end{subfigure}
\end{figure}

In addition to electron density, we can also estimate the [NII] 122 $\mu$m line intensity using the relationship between the two [NII] lines \citep{goldsmith2015herschel}.
...
Figure \ref{carina/fig:ratios} shows the ratio of the two lines at the calculated electron temperature for Carina Nebula, which is approximately 5660 $\pm$ 313.7 K.
...
Figure \ref{carina/eq:intensity_scatter} shows a scatter plot of the two lines, showing an overall trend of less [NII] in the Keyhole Nebula compared to the Carina Extended region.
...
We use a non beam corrected version of the [NII] 205 $\mu$m line to calculate the integrated intensity of the [NII] 122 $\mu$m line as we are interested in what our measurements would be on SOFIA if we had observed the [NII] 122 $\mu$m line as well.
...
% \begin{figure}[h]
%     \centering
%     \includegraphics[width=0.4\textwidth]{figs/carina/curve.png}
%     \includegraphics[width=0.4\textwidth]{figs/carina/calculated_curve.png}
...
% \begin{figure}[h]
%     \centering
%     \includegraphics[width=\textwidth]{figs/carina/ne_map_keyhole.png}
...
% \begin{figure}[h]
%     \centering
%     \includegraphics[width=\textwidth]{figs/carina/ne_map_carina.png}
