...
This is done by utilizing NumPy to cast the output as a \texttt{np.uint8} array and then returning a view of that array with big-endian 16-bit unsigned integer data type \cite{numpy}.
...
The specific cooler we are using was, fortuitously, a leftover prototype from the ACTDP that was not selected for the JWST mission and can be repurposed for ASTHROS \cite{kawamuraterahertz}.
...
This motor is controlled using the intelligent motion systems MDrive Motion Control System \cite{mdrive}.
