...
The readout system also communicates with other hardware on the payload, such as the power supply units and antenna systems, to monitor their status and control their operation.
This readout system is designed to be modular and scalable, allowing for easy integration of new detectors and readout systems.
Each package is designed to be self-contained, focusing on a single device in the readout system so that changes to the hardware can be made without impacting the rest of the system.
...
Without developing a readout system, the on-board autonomy work in Chapter \ref{ch:spectra} would have nowhere to run and no data to process.
...
Each piece of hardware on the payload required the development of custom drivers to wrap the native interface of the hardware in a common structure so all devices on the network could communicate with each other with a common protocol.
This design allows for easier integration of new devices and subsystems into the readout network, at the trade-off of additional complexity during initial development.
...
For the Raspberry Pi Compute Module 4 (CM4), the process of setting up the device to run the readout system is detailed, including the necessary packages and libraries that need to be installed, as this system is often used in a standalone setting with the \texttt{PyMCC} package.
...
This package is used in every device on the readout network to communicate with the RabbitMQ server to send messages and receive commands.

Following that, we detail the modules developed to run subsystems on the readout network.
For our science data, we describe the Pacific MicroCHIP Corporation P19800B ASIC RF Spectrometer, or PMCC, which is the primary detector used on the ASTHROS payload and the \texttt{PyMCC} package that was developed to control the PMCCs and read out the data.
Following that, we describe our Cryocool control system used to control the cryocooler that cools our detectors.
Next, we describe the Antenna controls used to monitor the antenna temperature and control the secondary hexapod and the Motor controls that control a small stepper motor used to chop between the sky and a blackbody reference.
All of these systems are controlled by a custom-built Power Supply Unit (PSU) with functionality wrapped to allow us to control the network.
Finally, we describe the Variable Attenuators in both their hardware design and the firmware used to control them.
...
Other harmonizing modules may be added in the future, but these two are the primary modules that are used to control the payload and monitor its status.
Finally, we conclude with a summary of the next steps for the system and the plan for integration of Chapter \ref{ch:spectra} with the readout system on the Analysis computer. 

\begin{figure}
    \centering
    \includegraphics[width=0.5\linewidth]{figs/spectra/system.png}
    \caption[High Level System Diagram for ASTHROS]{A high level overview of the ASTHROS Readout Network System Architecture showing the Command, Storage, and Analysis computers. The Command computer is responsible for sending commands to other processes and receiving status updates from the various publishers on the RMQ network. The Storage computer is responsible for storing all science data and telemetry data collected during the flight. The Analysis computer is responsible for analyzing the science data collected during the flight. The overview also shows some of the various subsystems and how they are connected in the network.}
    \label{readout/fig:system}
\end{figure}

Beyond ASTHROS, this readout system is designed with reusability in mind so that other missions using similar hardware can use the same software packages to control their payloads.
In particular, the package developed for controlling the spectrometers, \texttt{PyMCC}, is designed to be mission agnostic, with configuration files that can easily be adjuted.
Other modules are tied to custom ASTHROS hardware, such as the power supply units and antenna systems, but if other missions adopt similar hardware the packages can be used with little modification.
While these modules may have niche uses, we plan to open source the code and documentation so that other missions can use the software to control their payloads.

In to sharing the individual control packages, we have already open-sourced the \texttt{RMQTools} package that is used to communicate with the RabbitMQ server.
This framework has received  interest from the community for integration into other balloon borne missions so we made it available on GitHub with an MIT License\footnote{\url{https://github.com/217690thompson/rmqtools}}.
While it has not yet been used in other missions, we hope to conitnue development of the package and use it beyond ASTHROS to slowly gain adoption in the community and help support a more modular approach to payload readout systems.
...
The first is the command computer, a WinSystems flight computer, which is our primary command and control computer.
...
The second type of device is the a Storage computer, a custom RTD Embedded Technologies PC/104 stack.
...
In our configuration, the PC/104 stack consists of a main compute board, two managed network switches, three bays for solid state drives (SSDs), and a power supply board.
...
We utilize four 4 TB SSDs in either a RAID 5 or 10 configuration to provide a total of 12 or 8 TB of storage for the science data.
The decision between RAID 5 and 10 is based on the our level of redundancy and storage requirements. 
With RAID 10, we have a total of 8 TB of storage, but we can survive at least a single drive failure without losing any data, with the possilbity of surviving two drive failures if the failed drives are not in the same RAID 10 pair.
With RAID 5, we would have more storage at 12 TB but would only be able to survive one drive failure, as the parity data is spread across all drives.
...
This allows systems to mount the NAS and save data directly to the NAS without worrying about the underlying storage system.
...
As a proof of concept, we've developed an Odoo server that uses a PostgreSQL database to store housekeeping. 
...
The third system is the Analysis computer, a NVIDIA Jetson-based system responsible for analyzing the science data collected during the flight.
The Analysis computer is connected to the Storage computer via a wired Ethernet connection and runs the on-board autonomy system that processes the science data and sends it to the ground control system.
...
Finally, we have the CM4s that are used to control the PMCC spectrometers. 
...
Raw data is not sent through the RabbitMQ network in order to reduce the load on the system for non control critical data. 
...
As the CM4s are often used in a standalone setting, they can be configured to run independent from the rest of the readout network when not connected to the Storage computer.
...
To work around this, the BLAST readout system was built as a single monolithic program that orchestrates all the devices on the network \parencite{gordon2019highly}.
This approach is not scalable and makes it difficult to debug and maintain the system.

For ASTHROS, we wanted to build a modular system, consisting of microservices that could be easily integrated into the rest of the system using a shared protocol.
To accomplish this, we needed to decouple the devices so that data production and consumption are separated.
...
RabbitMQ is an open-source Advanced Messaging Queue Protocol (AMQP) implementation that serves as a broker for incoming and outgoing messages by routing messages to bound destination queues located on an exchange \parencite{dunne2018comparison}.
We chose RabbitMQ because of the centralized message broker architecture that allows for easy integration of new devices and services by networking all of our processes together.

The core of RabbitMQ is explained in this documentation as well as \cite{toshev2015learning}.
...
In our case, a message is any binary or blob of data, but we typically use JSON-formatted strings.
...
This is done using the routing key that specifies the queue to which the message should be delivered.
...
Typically, producers do not send messages directly to queues. 
...
\subsection{RMQTools}
While RabbitMQ serves as a powerful backbone for our networking infrastructure, we needed to devise a way for devices to communicate with the RabbitMQ server.
To accomplish this, we developed a Python package called \texttt{RMQTools} that provides a interface for creating both publisher-to-subscriber systems as well as Remote Procedure Call (RPC) frameworks.
...
Pika is a single-threaded RabbitMQ client library in Python that implements the AMQP protocol.
...
As a basic example of publisher-to-subscriber systems, we instantiate the \texttt{RmqConnection} class as an \texttt{rmq} object with the RabbitMQ server address and port as well as the username and password for the server.
We can then use \texttt{@rmq.set\_status\_exchange(name)} to specify which status exchange we will be using. 
These are done on both the publisher and subscriber side, ensuring that both are using the same server and exchange. 
Once we have configured the setup, we can implement a publisher using the \texttt{@rmq.publish\_status(interval, routing\_key)} decorator around a function that returns a dictionary.
...
We accomplish this by creating a new thread that creates a connection to the RabbitMQ server, sets the exchange, and creates a \texttt{Publisher} with the specified routing key.
After initalization, the function is run at the specified interval, and the resulting dictionary is published to the RabbitMQ server.

The subscriber is also implemented using a decorator, \texttt{@rmq.subscribe\_status( queue\_name, routing\_key(s))}.
The wrapped function requires four arguments: the channel, the method, the properties, and the body.
...
The body is the actual method that is received from the RabbitMQ server in the form of a JSON string. 
The decorator-wrapped callback function can then process the message in whatever way it needs to.

For RPC executions, we have implemented a similar system using the \texttt{RmqConnection} class.
...
The \texttt{ResponseObject} is a primitive class that contains args and kwargs attributes that are used to send the response back to the caller.
...
This works well for threaded applications as the decorated functions will run in a separate thread from the main thread once all threads are started. 
...
For most of our use cases, we would like to be able to control when these commands are sent, so we often times use the backend \texttt{\_send\_command()} helper function to manually send the command to the RabbitMQ server.
This works similarly to the decorator's use of \texttt{@rmq.handle\_response()} to decorate functions to handle responses.
Instead of starting a thread to send the command, we call the \texttt{rmq.\_send\_command()} function with a lambda function that creates the \texttt{ResponseObject} for the specific command we want to send.
...
While not specific to RabbitMQ, as a result of the threading model we use, we needed to implement a way to lockout commands from being sent to hardware while the hardware is in use by another thread. 
One example is when a publisher thread is collecting data to send a status update and a command is being sent through an RPC execution.
Locks prevent conflicts on the serial or SPI bus when two threads are trying to send bytes to the same device at the same time.
In addition to protecting the device, we also need to ensure that high-priority commands, like immediate commands from users, are not blocked by low-priority commands, such as sending a status update.
Thus, we created a priority lock system that allows us to create a lock object that we can then use to decorate functions that may be called at the same time.
...
In initialization, we create a boolean, \texttt{data\_free}, to determine if the data is free, a \texttt{queue\_lock} lock object using the \texttt{threading} library, and an array of \texttt{queue} objects whose length is equal to the number of priority levels.
\texttt{queue\_lock} is not used to protect the hardware but instead used to protect modification of the queues and the \texttt{data\_free} boolean.
When a function is decorated with \texttt{PriorityLock.with\_lock()}, the function will not execute until the \texttt{PriorityLock} allows the execution.
The decorator takes in the priority level of the function, a timeout length, and optional arguments for return values in case of a timeout or rejection. 
...
We now acquire the \texttt{queue\_lock} and check \texttt{data\_free} to see if there is another thread using the resource. 
If the resource is free, we set \texttt{data\_free} to \texttt{False} and return \texttt{True} to the decorator.
If the resource is not free, we create a new \texttt{Event} object and put it in the queue of the requesting priority level.
...
\texttt{remove\_event()} needs to be a bit more complex than removing the event from the queue in order to ensure that the queue doesn't change while we are removing the event.
...
We move all events except the one we are removing into the new queue.
...
If there are any events in the queue, we get the most recent event, set it, and return it.
...
\section{PMCC}
For ASTHROS, we utilize an array of 4GHz spectrometers called the PMCC ASIC P19800B ASIC RF Spectrometers (PMCCs).
...
Additionally, the CM4 has an SSD mounted to the side of its IO board enclosure for raw spectra storage and easier local debugging when the device is not connected to the rest of the readout network.
Finally, two CM4s and eight PMCCs are mounted in a custom enclosure that is designed to be mounted on the back of the ASTHROS primary mirror with the rest of the readout system.
...
Originally, the PMCCs were controlled by a C program that was designed to provide a basic CLI for manually controlling the PMCCs.
...
\subsection[spi\_utils]{\texttt{spi\_utils}}
...
When sending a command to the PMCC, the first word of the command is the address of the register we want to write to shift left by 1 bit to make room for the read/write bit.
\begin{equation}
...
For basic reads, \texttt{PMCC\_SPI} has a \texttt{read()} method that takes the address of the register we want to read from and, optionally, the number of words we want to read.
...
If we are only reading a single word, we return the first and only word in the array of words returned by the \texttt{xfer()} method. 
Otherwise, for burst reads, we return the entire array of words.

Oftentimes, such as with data ready flags, we want to read a specific value over and over until that value is True.
Many status bits on the PMCC operate in this way to indicate when a specific operation has been completed. 
...
For writes, \texttt{PMCC\_SPI} has a \texttt{write()} method that takes the address of the register we want to write to and the data we want to write.
...
Next, we check if the value provided will fit within the length specified by the MSB and LSB.
...
Finally, we provide a \texttt{close()} method that calls the \texttt{close()} method on the \texttt{spidev} object to close the SPI connection.

\subsection[config]{\texttt{config}}
There are a number of device-specific configurations that need to be set for each PMCC in order for it to operate correctly.
...
This YAML file has information about the RMQ configuration as well as the spectrometer configuration. 
For now, we will focus on the spectrometer configuration and discuss the RMQ configuration in Section \ref{readout/section:spec_rmq}.
...
These configurations are used to set values we do not expect to change individually for each PMCC.
...
The wiring for this PMCC is the first index in the wiring array that will still work with the rest of the system.
...
When loading in the configuration file, we create a \texttt{PMCC\_Config} object that contains all configuration information necessary for the CM4. 
...
\subsection[consts]{\texttt{consts}}
The \texttt{consts} module is a collection of constants used throughout the \texttt{PyMCC} module.
...
\subsection[driver]{\texttt{driver}}
...
Following SPI setup, the user must call the \texttt{initialize\_gpio()} method to set up the GPIO pin for the reset signal.
...
The PLL is used to synthesize all required clocks for the PMCC using an external reference clock. 
...
This is because we actually want to override the registers that are not set to 0.
...
Reading the \texttt{CHIP\_ID} register is a good way to verify that the SPI connection is working.
...
For the gain calibration, we have to set the \texttt{adc\_gain\_cal\_en} to 1 to enable the gain calibration.
...
In the instance where we already know the values we want to set for the gain and offset, we provide a \texttt{calibrate\_adc\_preset()} method that takes arrays of the 23 gain and 23 offset values to set the calibration values.
...
The \texttt{adc\_gain\_cal\_mode} and \texttt{adc\_offs\_cal\_mode} registers also need to be set to 0x1 and 0x2 to enable the use of the preset values.

After calibrating the ADC, we are able to configure the DSP. 
This is highly subjective to the experiment being run, but for ASTHROS, we have a specific configuration that works for the integration time and resolution we are using. 
In future versions of the code, we will likely pull out some of the hard-coded values and make them configurable in the YAML file.
...
\begin{enumerate}
    ...
    \item 
        ...
        We do this by dividing our desired integration time by the length of time it takes to run a single accumulation on the DSP. 
        For the ADC running at 8 GS/s (Gigasamples per second) reading 16384 time domain bins, this number is 2.048 $\mu$s.
    ...
    \item 
        ...
        We get eight because there is a maximum output resolution of 40 bits, and the value we can shift the most is 32.
    ...
    \item 
        ...
        \end{itemize}
        The combination of these skips every other stage of the IFFT and FFT processor, increasing performance for band-limited noise measurements.
...
The \texttt{retrieve\_data()} method follows the following process to read a spectrum from the DSP:
\begin{enumerate}
    \item 
...
\begin{enumerate}
    ...
    This is because each of the 20 core's readouts is a 6-bit value split across the 8 words in the line. 
...
These are mostly used for debugging and are not currently used in the normal operation of the PMCC.
In the future, we may want to have save the state of the PMCC to a file and load it back in to restore the PMCC to a known state after a reset or power cycle.
We do not currently do this as some registers must be set in a specific order to ensure that the PMCC is configured correctly.

\subsection[data\_utils]{\texttt{data\_utils}}
\texttt{data\_utils} provides classes to handle storing data from the PMCC. 
...
Finally, there is a \texttt{get\_files()} method that returns all the files in the directory where the data is stored.
...
This results in 2048 lines of data with 20 values for each core in each line.
...
When initializing, we create the directory if it does not exist. 
...
If a file is already linked to the writer, we close that file and open a new one.
...
\begin{table}
    ...
    \end{tabularx}
    \caption{Attributes of the Data Portion of the HDF5 File}
...
If the file does not exist, or we have reached a maximum number of writes, we call the \texttt{new\_file()} method to create a new file.
...
The \texttt{PMCC\_CSV\_Spectra\_Writer} also implements \texttt{write\_spectra()} and \texttt{new\_file()} methods so that they can be used interchangeably with the \texttt{PMCC\_H5\_Writer} class.
...
This method also increments the count of the number of spectra in the file, the total number of spectra written by the writer, and will create a new file if one does not exist or we reach the maximum number of spectra in a file.
...
This is useful for writing data to both HDF5 and CSV files at the same time as well as writing data to multiple locations, such as locally on the CM4 and remotely on the NAS. 
\texttt{new\_file()} and \texttt{write\_spectra()} call the same methods on all of the writers in the array.
...
\label{readout/section:spec_rmq}
...
This complexity has led us to develop a threaded approach to controlling the PMCCs that interact with the threads from the RabbitMQ control loop.
...
For most atomic methods, such as \texttt{pll} to initialize the PLL or \texttt{reset} to reset the PMCC, we run the command and then return the result. 
...
\texttt{take\_spectra()} is a while loop that runs while the collecting flag is set. 
While running, it uses the driver's \texttt{retrieve\_data()} method to collect spectra and pass that data to the \texttt{PMCC\_Spectra\_Writers} object to write the data to the file.
...
If the flag is already set or the thread is alive, we return False and set an error stating that the spectra are already being collected.
...
Once joined, we create a new thread to start collecting spectra again if the user calls \texttt{start\_spectra()}.
...
In this case, we add the error to the error buffer and return False, indicating that the spectra were not stopped correctly.
If the flag is also not set, we know that \texttt{stop\_spectra()} was called before \texttt{start\_spectra()}, and we return False, indicating that no spectra were being taken.
...
\begin{itemize}
    ...
    \item \texttt{read} - Read the PMCC registers using the \texttt{fetch\_registers()} method.
    \item \texttt{write} - Write to the PMCC registers using the \texttt{write\_registers()} method using a dictionary of register-value key-value pairs.
    \item \texttt{ack} - Toggle the PMCC acquisition bit using the \texttt{pulse\_acquisition()} method.
...
The specific cooler we are using was, fortuitously, a leftover prototype from the ACTDP that was not selected for the JWST mission and is repurposed for ASTHROS \parencite{kawamuraterahertz}.
...
\subsection[cryocool]{\texttt{cryocool}}
\begin{table}
    \centering
    \begin{tabularx}{\textwidth}{l|l|X}
        \textbf{Key} & \textbf{Type} & \textbf{Description} \\ \hline    
...
We then make a note of the status registers that are read out when we issue a status command to the cryocooler.
...
Each row of the table is read in as a \texttt{Reg} object that allows us to easily convert between the raw register values and the intended units.
...
This is accomplished by a linear conversion as shown in Equation \ref{readout/eq:reg_convert}.
\begin{equation}
...
\end{equation}
Additionally, if the value is converted to a count, we round the result to the nearest integer.
...
The \texttt{Reg} object, despite knowing if a register is a temperature value or not, does not convert the temperature values to Kelvin.
...
Once the \texttt{Cryocool} object is initialized, the \texttt{Reg} objects are created and stored in a dictionary for easy access.
...
Commands sent to the cryocooler are done using the serial library in Python.
To support the serial communication, we have implemented \texttt{serial\_write()} and \texttt{serial\_read()} methods to act as the lowest level interface for the \texttt{get()}, \texttt{set()}, and \texttt{status()} methods.
The \texttt{serial\_write()} command takes in a byte array and writes to the serial buffer.
...
This method takes no arguments and reads the serial buffer until it hits a predetermined timeout or we stop receiving bytes.
...
Then we unstuff the bytes and check the checksum. 
...
The returned value will be the raw digital value from the register, which is converted to the intended units using another method.
...
\begin{enumerate}
    ...
    \item Byte stuff every \texttt{0x01} after the start frame delimiter.
\end{enumerate}
...
The cryocooler has a built-in status command that reads 14 status registers at once.
...
With all of the serial communications methods in place, we can now build on top of those to create the user-facing methods.
...
This method can take a \texttt{Reg} object, a label, or an address as an argument.
...
The result after the conversion of temperature registers will be in Voltage.
...
The \texttt{in\_kelvin} and \texttt{convert} parameters specify if the value needs to be converted from temperature to voltage and if the value needs to be converted to a digital count, similar to the \texttt{get\_data()} method.
...
Instead, it takes the optional parameters for \texttt{convert} and \texttt{in\_kelvin} to convert the status registers to the intended units.
\texttt{get\_status()} calls the \texttt{status()} method to read the status registers and then iterates over the \texttt{TELEMETRY} registers specified in the configuration file to match the read values to their \texttt{Reg} objects. 
...
This method is used to get a snapshot of the cryocooler's state and to debug and monitor the health of the cryocooler during the flight. 
We first read the status registers using the \texttt{get\_status()} method.
We then iterate over all registers in the configuration file and read the value of each register that was not read in the status command.
...
\subsection[cryotemp]{\texttt{cryotemp}}
...
The configuration for the Lakeshore devices is stored in the YAML file that contains the device's connection parameters.
...
The class then implements three methods to read the values from the Lakeshore device.
...
\subsection[pressure]{\texttt{pressure}}
The \texttt{pressure} module is used to read data from the pressure sensor on ASTHROS.
The pressure sensor is a single device connected to the central command computer via RS-485.
...
\subsection{RabbitMQ Control Loop}
Using the drivers in the \texttt{cryocool}, \texttt{cryotemp}, and \texttt{pressure} modules and the \texttt{rmqtools} package, we can create a control loop for the cryocooler to interact with the rest of the system.
...
The key \texttt{LAKESHORE} in the configuration file is links to an array of configurations for each Lakeshore device, so we need to iterate over the array and create separate \texttt{Lakeshore} objects for each device, mapping them to the dictionary using \texttt{LABEL} as the key.
...
First, we initialize a \texttt{PriorityLock} so that we do not end up with multiple threads trying to access the same hardware at the same time.
...
These functions are wrapped with \texttt{rmq.publish\_status()} at intervals of 20 seconds, offset by 5 seconds for each function to stagger the responses. 
...
This is returned in the \texttt{rmqtools} wrapper to be packaged and sent back to the sender.

After defining all of these functions, we run \texttt{rmq.start()} to start the threads and begin listening for commands.

\section{Antenna}
ASTHROS flies with a 2.5m diameter antenna to collect far-infrared light. 
...
This computer is connected directly to the central command computer via RS-485 and to the rest of the system via Ethernet through the readout network.
...
In the case that we receive an error from the antenna, we can check the error code against the \texttt{ALARMS} and \texttt{NACK} arrays to determine the error.

For communication with the antenna, our lowest level interface is the \texttt{decode\_read()} method, which reads whatever is in the serial buffer and returns the byte array.
...
\texttt{read\_all()} iterates over all of the registers in the configuration file, checks if the register has a read address, and then runs \texttt{read\_reg()}.
...
Then, we iterate through all of the fields in the register, checking if they are in the fields passed to the method and adding them to the payload.
...
This is decorated with \texttt{rmq.publish\_status()} at a one second interval and the lowest level of priority lock.
...
These values are packaged and handled exactly like the \texttt{antenna\_status()} function and published to the status exchange with the queue name \texttt{pointing.antenna} on the status exchange.
...
\begin{itemize}
    \item \texttt{read} - Read a specific register based on an ID label in the configuration file.
    \item \texttt{write} - Write to a specific register using the ID label in the configuration file and keyword arguments for the fields to write.
    \item \texttt{read\_write} - Perform a read and write on a specific register using the ID label in the configuration file and keyword arguments for the fields to write, falling back to the read values if not provided.
    \item \texttt{specsheet} - Return a listing of all of the registers in the configuration file along with their descriptions and ability to read and write. Useful when you forget the label of a register.
\end{itemize}
...
While we could use the MDrive software to control the motor, we have implemented a Python interface to control the motor to better integrate it with the rest of our architecture. 
...
Both of these methods will be explained after discussing the lowest level interface for serial communication with the motor.
...
The \texttt{write()} method can also take an optional \texttt{readwait} parameter that will read the response from the motor after writing the command using the \texttt{readall()} method.
The \texttt{readall()} method reads the serial buffer until it hits a timeout or stops receiving bytes.
It then takes the byte array from the read and decodes it into a string.

To reset the motor, we have implemented a \texttt{reset()} method that sends a reset command to the motor.
This command is the string \texttt{\textbackslash x03}, which is the ASCII character indicating the end of the text.
...
During initialization, we take the \texttt{HWVARS} dictionary from the configuration file and pass that as a parameter to the \texttt{set\_hwvars()} method.
\texttt{set\_hwvars()} iterates over the dictionary provided to set multiple variables and calls the singular \texttt{set\_hwvar()} for each key-value pair.
\texttt{set\_hwvar()} takes in a key and value and writes the command to set the hardware variable to the motor as \texttt{<key> <value>}.
...
For ASTHROS, we have a few predefined functions we use to interact with the motor, shown in Table \ref{readout/table:motor_functions}.
...
To begin, we load the configuration for the motor and RabbitMQ from the YAML file.
...
The first is \texttt{motor\_status()} which is decorated with both the \texttt{rmq.publish\_status()} and \texttt{lock.with\_lock()} decorators outlining a 15 second interval for publishing and the lowest level of priority.
The function calls \texttt{motor.status()} and packages the response as a dictionary with the \texttt{status} and \texttt{time} keys.
...
\begin{itemize}
    \item \texttt{execute} - Execute a function on the motor based on the label in the configuration file. 
    \item \texttt{write} - Write a specific hardware value to the motor based on the passed variable and value.
    \item \texttt{status} - Receive the status of the motor from \texttt{motor.status()}
\end{itemize}
...
These tools handle most of the low-level details of packetizing and parsing the data from the Smart PSU so that end users can easily interact with it. 
...
As such, we have developed and implemented a Python package that wraps the command line tools as a driver for the Smart PSU so that it can be integrated into the rest of the readout system. 

The core of the design for the ASTHROS Smart PSU drivers is the \texttt{SmartPsu} class that contains a set of \texttt{channels} through which the Smart PSU can be controlled.
...
The \texttt{Environment} object is a class that contains a set of paths to the tools as well as setting up a logging system that other classes can use to log messages, warnings, and errors. 
...
The \texttt{read()} method reads the channel using the \texttt{read\_voltage\_current\_tool} defined in the \texttt{Environment} object and returns the voltage, current, and enabled status of the channel.
...
When \texttt{Channel} is set up within the \texttt{SmartPsu} class, minimum and maximum allowed voltages are defined for the channel such that any \texttt{safe\_write()} will validate the requested voltage is within the allowed range and raise an exception if it is not.
...
The configuration file can be either a CSV or an INI file. CSV is the preferred format, but INI files are supported for backward compatibility with the original implementation.
...
We utilize the Micro Lambda Voltage Controlled Oscillator (MLVS) based synthesizer in our readout pipeline \parencite{mlvs}. 
...
\begin{itemize}
    \item \texttt{get\_ID()} - Return the ID of the synthesizer.
    \item \texttt{get\_freq()} - Return the frequency of the synthesizer.
    \item \texttt{set\_freq()} - Set the frequency of the synthesizer between 500 and 20000 MHz.
    \item \texttt{get\_status()} - Return the status of the synthesizer.
    \item \texttt{get\_temperature()} - Return the temperature of the synthesizer.
    \item \texttt{get\_highest\_temperature()} - Return the highest recorded temperature of the synthesizer.
    \item \texttt{run\_self\_test()} - Return the results of a self-test on the synthesizer.
    \item \texttt{get\_ref\_source()} - Return the reference source of the synthesizer (e.g. \texttt{INT}, \texttt{EXT}).
    \item \texttt{set\_ref\_source()} - Set the reference source of the synthesizer (e.g. \texttt{INT}, \texttt{EXT}).
\end{itemize}
...
For human readibility, we also have a \texttt{get\_temps()} method that maps the sensor IDs to their labels and returns a dictionary of the labels and their associated readouts.

\subsection{Command Line Interface}
Oftentimes, during testing, we need to run the Smart PSU without the overhead of having the RabbitMQ control loop running.
This is especially true when we do not have the complete network setup or are testing specific components on the bench, detached from the physical payload. 
In a previous iteration of the Smart PSU control system, we had a command line interface that allowed us to issue commands to the Smart PSU and get responses back.
...
\begin{table}
    ...
    \begin{tabularx}{\textwidth}{l|X}
        ...
        \texttt{checkpart\_RFoff} & Check the status of a part connected to an output based on the specified current nominal values and tolerances with RFoff. \\
        \texttt{checkpart\_RFon} & Check the status of a part connected to an output based on the specified current nominal values and tolerances with RFon. \\
...
\subsection{RabbitMQ Control Loop}
Unlike the driver implementation and the Command Line Interface, which were developed to function exactly like the original implementation for backward compatibility, the RabbitMQ Control Loop for the Smart PSU wraps the driver in a more modern interface that allows for easier integration with the rest of the system.
In other implementations on the system, we have initialized our drivers with a YAML configuration file that contains all the necessary parameters to set up the driver. 
...
The flight control loop is set up using \texttt{rmqtools} and is similar to the other control loops in the system but has the added complexity of multiple Smart PSUs. 
We load in the configuration file and set the \texttt{RmqConnection} object as we would for any other control loop.
Within the main loop, we implement a \texttt{PriorityLock} to prevent collisions when writing to the Smart PSU.
This is especially important given we have multiple Smart PSUs that can be controlled at the same time, resulting in multiple threads that may potentially be trying to write to the same Smart PSU.
For RPC commands, we set up two threads, one for the \texttt{SmartPsu} object and one for the \texttt{SPIReadWrite} object, which primarily controls the synthesizer.
...
These methods will be set up with a staggered delay to prevent all of the threads from executing at the same time when \texttt{rmq.run()} is called. 

Finally, all of these decorated methods are set up with the same \texttt{PriorityLock} to prevent collisions when reading and writing to the Smart PSU.
...
As this is a standalone device on the system, it is controlled by its own Arduino Nano Every microcontroller and interacts with the rest of the system as a networked device using MQTT (Message Queuing Telemetry Transport) \parencite{mqtt}.
Each Arduino Nano Every is configured to control up to eight variable attenuators.
...
These multiplexers are given indexed addresses starting at 0 based on their position in the stack of slices. 
...
The \texttt{if\_amp} library is a library that utilizes the \texttt{Wire} library to provide intuitive functions to control multiple variable attenuators.
The lowest level interface in \texttt{if\_amp} is the \texttt{rw\_pca()} function.
...
The attenuator can only be set in 0.25 dB steps, so we multiply the input by 4 and round it to the nearest integer.
...
For testing, we have implemented a basic serial interface. 
...
To set up the networking, we give the Arduino a unique MAC address and a static IP address.
On ASTHROS, we have three of these systems, so we provide them with addresses based on their hard-coded ID. 
...
After setting up the Ethernet connection, we set up a client to connect to the MQTT broker.
...
We then attempt a connection to the broker through the client using the name of our device and an account on our RabbitMQ server setup to handle MQTT requests.
...
MQTT does not support direct addressing, so we work around this by sending a message to the topic exchange with the \texttt{routing\_ID} as the route. 
Whoever sends the request will need to subscribe to the \texttt{mqtt/comm/<routing\_ID>} topic to receive the response.
...
There is an error offset parameter that adds an offset to any returned errors.
...
If not, we perform a \texttt{single\_read()} with an offset of 100 to indicate any errors in reading back the value that came from the read operation. 
...
This is useful because if the system is not connected to the network, the MQTT interface will continually try to connect and fail.
At a predefined interval, the system will publish a message to the \texttt{mqtt/comm/<ID>} topic with the status of the system.
This is integrated into our state machine while checking for MQTT messages by comparing the last time we published a status message with the current clock time and setting the state to \texttt{READ} and system to \texttt{MQTT} if the time has elapsed.
...
In our initial implementation of housekeeping, we intended to conform to the structure necessary for downstream processing using Ground Support Equipment Operating System (GSEOS) \parencite{hauck2003use}.
...
Due to changes on the gondola's software side, the housekeeping system will need to be modified to conform to a different framework.
...
The housekeeping system is the culmination of every other system in the readout pipeline.
Every system previously defined produces a status message that needs to be captured, analyzed, and forwarded to the operations team.
...
Because we have a single pipeline for all of our data, we need to segment and packetize the data into different blocks so that we do not exceed our downlink bandwidth.
...
The \texttt{Housekeeping} library is a set of data queues in a fixed sequence, where each data queue is checked in a predefined order on a set interval.
...
Blocks are set up with a \texttt{msg\_id} signifying the type of data being processed. 
...
Finally, we have a \texttt{data\_queue} object, which is a Queue that contains encoded data that is ready to be sent to the socket.

Each block then has a set of methods that are used to acquire and process the data.
...
As each subsystem may need a different method to handle incoming messages, each block has its own \texttt{subscriber()} method unique to the block's datatype.
...
Before we get to the main loop, where our sequenced clock sends data at regular intervals, we need to set up the blocks to receive data. 
...
Afterward, we sleep until the next tick of the clock and repeat the process.
...
The antenna system contains values read from the antenna.
There is a single \texttt{housekeeping.antenna} routing key to subscribe to, and the data is a dictionary packed with various readout values from the antenna.
...
The cryocooler block is similar to the antenna block in that it is a dictionary of values from the cryocooler system.
...
Within the \texttt{subscriber()} method, we check the routing key of the incoming message and handle each one accordingly. 
Because our \texttt{QueueTTL} object handles the TTL of each key, we \texttt{put()} the data into the \texttt{data} object, and the logic for handling stale data is handled when we call \texttt{get()}. 
...
The IF block is one of the more basic blocks in the system.
Unlike the other systems, the IF systems do not use the traditional \texttt{status} exchange as it operates using the MQTT protocol. 
...
Each slice has up to eight channels, except for slice 0, which only has one for the 100GHz receiver. 
There are a total of 17 channels that we need to monitor.
The data from the IF block is a basic dictionary of attenuation values in dB as well as the status flag from each channel.
...
Because we have multiple Smart PSUs, we actually have multiple \texttt{PsuBlock} objects in the sequencer so that we can have separate threads for each PSU and spread out the bandwidth to the GSEOS socket.
...
The data from the Smart PSUs are the voltages, currents, and enabled status from each channel, as well as some overhead data for the PSU itself.
...
\begin{itemize}
    ...
    \item \texttt{Synth\_ID} - The ID of the synthesizer, which should be "MLVS-1520AS" and is downlinked for verification we have an SPI connection.
...
We do not need to split the temperature block into multiple blocks, as we are able to fit all of our combined data into a single block.
For routing, we have six routing keys for the six power supplies on the gondola, \texttt{housekeeping.temp.<psu\_id>} where \texttt{<psu\_id>} is the ID of the PSU.
...
Other than that, we iterate over the sensors in the configuration file and use the ID as a key in the \texttt{data} object to order and encode each value from -327.68 to 327.67 C.
...
This led us towards \texttt{click}, a Python library for building command line applications \parencite{click}.
\texttt{click} enabled us to create a custom command line interface (CLI) that consolidated all of our systems into a single interface.
...
To accomplish the first of these two goals, we utilize \texttt{click}'s ability to add groups and subcommands to the CLI.
We call these \texttt{plugins} and developed a plugin for each subsystem that we needed to control. 
...
In a typical implementation, each command sent is atomic, where each command's execution exists in a vacuum, but we can work around this by storing a persistent \texttt{Controller} in within \texttt{click.get\_current\_context()}. 
...
Terminal mode is a special feature of the CLI that uses a REPL to allow the user to interact with the system in a more natural way, without having to type out \texttt{python central\_command.py --config config.yml ...} every time.
...
To match the flight system, we have implemented a version of Central Command that listens to a socket and receives strings from gondola.
...
The biggest challenge here is capturing the output of the command and sending it back to the gondola.
...
\texttt{seq} is a bespoke scripting plugin that allows us to write a sequence of commands to be executed in order as a text file. 
...
Likewise, the \texttt{fabric} plugin is an interface that allows us to execute commands on any computer on the network.
...
We are still discussing appropriate signal to noise ratios (SNR) for the snapshot data products, but methods to perform this are already implemented. 
Both the Level 1 and Level 2 data product prototypes have been created but are not fully implemented in the readout system as an automated process on the Analysis computer. 
...
The theory behind our approach is outlined in Chapter \ref{ch:spectra} but we have not yet implemented the system in the readout pipeline.
...
Calculating just using the OFF measurements would produce anomaly scores that are the most useful for mission operations as these calibration measurements will likely be the most stable as they are not dependent on the target being observed.

In addition to analysis steps, there are calibration tasks remaining with the PMCCs.
Currently, we are using the manufacturer's suggested default settings for the spectrometers and they perform well for most of our science goals.
