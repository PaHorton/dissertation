...
When calling the \texttt{rmq\_main.py} script, we pass three arguments: \textttt{config}, the configuration file, \texttt{group}: the group name, and an optional list of indexed \texttt{ids} to control.
...
After creating this dictionary, we manually create \texttt{threading} threads for the \texttt{rmq.\_publish_status()} decoration, passing \texttt{PMCC\_Thread.send_status()} as the target function at a 1 second interval. 
...
As described in \ref{chap2/section:rmqtools}, this decorated function will receive a string for the command name as well as any positional arguments or keyword arguments.
...
This YAML file outlines the registers on the antenna as shown in Table \ref{chap2/table:antenna_regs}.
...
Specifically, we utilize the F1956NBGI8 Attenuator from Renesas Electronics for adjusting our attenuation levels \citep{Renesas}.
...
A PCA9571 is used as an I$^2$C I/O expander to control the digital channels on the attenuator \cite{pca9571}.
These expanders all have the same address of \texttt{0b0100101X} where \texttt{X} is the read/write bit. 
Because of this, we utilize an LTC4305 to multiplex the I$^2$C bus to provide each expander with a unique address \cite{ltc4305}.
...
In our initial implementation of housekeeping, we intended to conform to the structure necessary for downstream processing using GSEOS (Ground Support Equipment Operating System) \citep{hauck2003use}.
...
\begin{itemize}
    ...
    \item \texttt{RC}- current as a percentage of the maximum current$^{\ref{chap2/footnote:motor}}$
    \item \texttt{A} - The acceleration of the motor in steps per second squared$^{\ref{chap2/footnote:motor}}$.
    \item \texttt{D} - The deceleration in steps per second squared$^{\ref{chap2/footnote:motor}}$.
    \item \texttt{VM} - The maximum velocity of the motor in steps per second$^{\ref{chap2/footnote:motor}}$.
    \item \texttt{VI} - The initial velocity in steps per second$^{\ref{chap2/footnote:motor}}$.
    \item \texttt{P} - The current position of the motor in steps.
    \item \texttt{CB} - The current control bounds, a mode selection for torque performance$^{\ref{chap2/footnote:motor}}$.
...
Table \ref{chap2/table:click_plugins} shows a list of the plugins that are currently implemented or planned for the system.
