...
Given the short time frame of tactical planning in which downlinked images need to be analyzed (within 5-10 hours before the next uplink), there exists a need to triage analysis time to focus on the most important sequences and parts of a sequence. 
...
As an example, a distant meteorite might be difficult to spot in a single RGB image, especially if it blends in with the surrounding terrain.
In the multispectral, the meteorite will be easily identifiable as it will have a different spectral signature than the surrounding terrain.
The goal of the novelty detection system is not to answer science questions but instead to triage potential science targets for mission operations staff to analyze.
This enables operations staff to spend more time on the most interesting targets and reduces missed opportunities for discovery.
...
MSLWEB, an Arizona State University-based Mastcam tracking application, is the ideal platform for integrating novelty detection products into tactical planning workflows. 
...
At the time of publishing this chapter, Mastcam on the MSL curiosity rover was still operational.
Since then, the filter wheel on the MSL Mastcam has failed, and the instrument is no longer able to take multispectral images.
That said, the novelty detection system still has value as a method that can be integrated into the Perseverance rover's Mastcam-Z instrument.
Being a currently operational instrument, Mastcam-Z is the new ideal platform for integrating novelty detection products into tactical planning workflows for further development of this system.
