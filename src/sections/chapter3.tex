...
This is done by appending the following to the end of the \texttt{/boot/firmware/cmdline.txt} file:
\begin{verbatim}
    spidev.bufsiz=65536
\end{verbatim}
...
After rebooting, we recommend connecting the CM4 to the internet through a wireless hotspot so we can install the necessary packages.
...
This is done by updating the \texttt{/etc/hosts} file with the IP addresses and hostnames in Appendix \ref{appendix:hosts}.
