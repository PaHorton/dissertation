...
\begin{itemize}
    \item \textbf{X00} - No error
\end{itemize}
From here on, the errors are sorted by the system that is reporting the error.
Errors with data validation:
\begin{itemize}
    \item \textbf{X01} - Bad Address Received
    \item \textbf{X02} - Bad Attenuation Received
\end{itemize}
Errors with connecting to the LCT4302:
\begin{itemize}
    \item \textbf{X11} - Buffer Overflow from LCT4302 Connection
    \item \textbf{X12} - NACK Received from LCT4302 connection
    \item \textbf{X13} - Bus Error or Arbitration Lost from LCT4302 connection
    \item \textbf{X14} - Timeout from LCT4302 connection
    \item \textbf{X15} - Unknown Error from LCT4302 connection
\end{itemize}
Errors connecting to the PCA9571:
\begin{itemize}
    \item \textbf{X21} - Buffer Overflow from PCA9571 Connection
    \item \textbf{X22} - NACK Received from PCA9571 connection
    \item \textbf{X23} - Bus Error or Arbitration Lost from PCA9571 connection
    \item \textbf{X24} - Timeout from PCA9571 connection
    \item \textbf{X25} - Unknown Error from PCA9571 connection
\end{itemize}
Errors disconnecting from the LCT4302:
\begin{itemize}
    \item \textbf{X31} - Buffer Overflow from LCT4302 Disconnection
    \item \textbf{X32} - NACK Received from LCT4302 disconnection
    \item \textbf{X33} - Bus Error or Arbitration Lost from LCT4302 disconnection
    \item \textbf{X34} - Timeout from LCT4302 disconnection
    \item \textbf{X35} - Unknown Error from LCT4302 disconnection
\end{itemize}